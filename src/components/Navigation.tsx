...
const Navigation = () => {
  ...
  const handleShare = () => {
    ...
    toast({
      ...
      description: "【GitHubエクスポートの手順】\n1. 画面上部の「Edit code」アイコン（鉛筆アイコン）をクリックしてください\n2. 表示されるポップアップ画面の一番上の入力欄に、リポジトリ名を入力してください（例：core-esports）\n3. その下にある「Public」（公開）または「Private」（非公開）を選択\n4. 一番下の「Export to GitHub」ボタンをクリック\n\n【URLの確認方法】\n・エクスポート完了後、画面上部に緑色の通知バーが表示されます\n・通知バーの中にあるURLをクリックすると、GitHubリポジトリが開きます\n・この通知バーは一時的に表示されるので、必要な場合はURLをコピーしてください",
      duration: 25000,
    });
  };
