    ...
    <section id="products" className="py-20 bg-navy-light relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 relative z-20">
        ...
        <div className="text-center mb-12 mt-16">
          <h3 className="text-4xl md:text-5xl text-gold font-bold mb-6 animate-glow drop-shadow-lg">
            Core指サックプレミアム
          </h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-4xl mx-auto px-4">
          {features.map((feature, index) => (
            <div
              key={index}
              className="bg-navy/80 backdrop-blur-sm p-8 rounded-lg hover:bg-navy/70 transition-colors text-center"
            >
              <feature.icon className="w-12 h-12 text-gold mb-6 mx-auto" />
        ...
        <div className="text-center mt-16 space-y-6">
          ...
          <Link
            to="/reviews"
            className="inline-flex items-center px-8 py-4 text-lg bg-white text-navy font-bold rounded-full border-2 border-gold hover:bg-gray-200 transition-colors group"
          >
            プロ選手の声
            <ArrowRight className="ml-2 w-6 h-6 group-hover:translate-x-1 transition-transform" />
          </Link>
